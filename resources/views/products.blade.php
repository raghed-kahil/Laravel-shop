      ...
      <div class="row">
        ...
        <div class="col-lg-3">
          ...
          *** MENUS AND FILTERS ***
          ...
          <div class="card sidebar-menu mb-4">
            ...
            <div class="card-body">
              <ul class="nav nav-pills flex-column category-menu">
                <li><a href="/products" class="nav-link">Men <span
                      class="badge badge-secondary">42</span></a>
                  <ul class="list-unstyled">
                    <li><a href="/products" class="nav-link">T-shirts</a></li>
                    <li><a href="/products" class="nav-link">Shirts</a></li>
                    <li><a href="/products" class="nav-link">Pants</a></li>
                    <li><a href="/products" class="nav-link">Accessories</a></li>
                  </ul>
                </li>
                <li><a href="/products" class="nav-link active">Ladies <span class="badge badge-light">123</span></a>
                  <ul class="list-unstyled">
                    <li><a href="/products" class="nav-link">T-shirts</a></li>
                    <li><a href="/products" class="nav-link">Skirts</a></li>
                    <li><a href="/products" class="nav-link">Pants</a></li>
                    <li><a href="/products" class="nav-link">Accessories</a></li>
                  </ul>
                </li>
                <li><a href="/products" class="nav-link">Kids <span
          ...
          <div class="card sidebar-menu mb-4">
            <div class="card-header">
              <h3 class="h4 card-title">Brands <button filter="brands" class="btn btn-sm btn-danger pull-right clear-filter"><i
                    class="fa fa-times-circle"></i> Clear</button></h3>
            </div>
            <div class="card-body">
              <form action="" method="post">
                <input type="hidden" name="filter" value="brands">
                <div class="form-group">
                  @foreach($brands as $brand)
                  <div class="checkbox">
                    <label>
                      <input name="{{$brand->id}}" type="checkbox" value="1" {{in_array($brand->id , $checkedBrands)?'checked':''}}> {{$brand->name.' ('.$brand->count.')'}}
                    </label>
                  </div>
                  @endforeach
                </div>
                <button type="submit" class="btn btn-default btn-sm btn-primary"><i class="fa fa-pencil"></i> Apply
                </button>
              </form>
            </div>
          </div>
          <div class="card sidebar-menu mb-4">
            <div class="card-header">
              <h3 class="h4 card-title">Colours <button filter="colors" class="btn btn-sm btn-danger pull-right clear-filter"><i
                    class="fa fa-times-circle"></i> Clear</button></h3>
            </div>
            @push('scripts')
              <script>
                $(document).ready(function () {
                  $('.clear-filter').click(function () {
                    $.ajax({url: '/products', type: 'POST', data: { 'clear': $(this).attr('filter')},
            ...
            <div class="card-body">
              <form action="" method="post">
                <input type="hidden" name="filter" value="colors">
                <div class="form-group">
                  @foreach($colors as $color)
                    <div class="checkbox">
                      <label>
                        <input name="{{$color->id}}" type="checkbox" value="1" {{in_array($color->id , $checkedColors)?'checked':''}}> {{$color->name.' ('.$color->count.')'}}
                      </label>
                    </div>
                  @endforeach
                </div>
              ...
              <div class="col-md-12 col-lg-7 products-number-sort">
                <form
                  ...
                  <div class="products-sort-by mt-2 mt-lg-0"><strong>Sort by</strong>
                    <select id="sort-order" name="sort-by" class="form-control">
                    ...
                    @push('scripts')
                      ...
                      <script>
                        $(document).ready(function () {
                          $('#sort-order').find('[value="{{Cookie::get('sort-by','')}}"]').attr('selected',true);
                          $('#sort-order').change(
                            function () {
                            $.ajax({url: '/products', type: 'POST', data: { 'sort-by': $(this).find(":selected").attr('value')},
